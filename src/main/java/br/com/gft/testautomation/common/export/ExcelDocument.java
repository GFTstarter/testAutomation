...
// CellRangeAddress is deprecated
public class ExcelDocument extends AbstractExcelView {
	...
	/**
	 * Method that creates a PopulateHeader object, setting the values received
	 * in the appropriated attributes.
	 */
	public void createHeader(long id_ticket, String jira, String release,
			String description, String environment, String developer,
	...
	/**
	 * Method that actually creates the MS-Excel document. This method came with
	 * the extending of the AbstractExcelView class
	 */
	public void buildExcelDocument(Map<String, Object> model,
		...
		SummaryInformation summaryInfo = PropertySetFactory
				.newSummaryInformation();
		summaryInfo = workbook.getSummaryInformation();
		summaryInfo.setAuthor("GFT Brasil");

		/*
		 * Instantiates objects that contain the custom style and the custom
		 * colours
		 */
		...
		// Content and style
		cellPanelHeader.setCellValue(populateHeader.getExcelJira()
				+ " Test Case");
		...
		cellPanel02.setCellValue(populateHeader.getExcelJira() + " - "
				+ populateHeader.getExcelDescription());
		// Merging the cells
		...
		int lin = 0;
		/*
		 * Uses the TestCases list to add data to the MS-Excel document. Each
		 * row of the list will be add to a new row at the document, using the
		 * addValueToCell method. For each row added, the method breakLine will
		 * be executed once, using the TestCase object.
		 */
		for (TestCases testCase : testCasesList) {
			...
			ExcelUtils.addValueToCell(
					row1,
					1,
					populateHeader.isFileBlank() ? "Pending" : testCase
							.getStatus(), cellStyleContent);
			ExcelUtils
					.addValueToCell(row1, 2, populateHeader.isFileBlank() ? ""
							: testCase.getTested_by(), cellStyleContent);
			ExcelUtils
					.addValueToCell(row1, 3, populateHeader.isFileBlank() ? ""
							: testCase.getTested_on(), cellStyleContent);
			ExcelUtils.addValueToCell(row1, 4, testCase.getPre_requisite(),
					cellStyleContent);
			ExcelUtils.addValueToCell(row1, 5,
					testCase.getTestcase_description(), cellStyleContent);
			ExcelUtils.addValueToCell(row1, 6, testCase.getResults(),
					cellStyleContent);
			ExcelUtils.addValueToCell(row1, 7,
					populateHeader.isFileBlank() ? "" : testCase.getComments(),
					cellStyleContent);
		}
		...
		/*
		 * Creates the MS-Excel document in a temporary folder, in case they are
		 * used by another method.
		 */
		// Create the folder
		File tempFolder = new File(tempPath + File.separator + "testapptemp"
				+ File.separator + "files");
		tempFolder.mkdir();
		// Create the file
		String fileName = populateHeader.isFileBlank() ? "_Test_Plan_blank.xls"
				: "_Test_Plan.xls";
		FileOutputStream fos = new FileOutputStream(tempPath + File.separator
				+ "files" + File.separator + populateHeader.getExcelJira()
				+ fileName);
		workbook.write(fos);
		fos.close();
	}
